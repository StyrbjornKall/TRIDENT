...
def CalculateWeightedAverage(df):
    ...
    for col in medians.columns:
        if col not in ['COMBINED_CAS','COMBINED_effect','COMBINED_endpoint', 'SMILES','SMILES_Canonical_RDKit','cmpdname','counts']:
    ...
    for col in mean.columns:
        try:
            if col not in ['COMBINED_CAS','COMBINED_endpoint','SMILES','SMILES_Canonical_RDKit','cmpdname','counts']:
